...
class FCN(nn.Module):
    ...
    def __init__(self, nin=2, nout=2, nh=[24, 24, 24], device=0):
        super().__init__()
        if type(nh) != list:
            nh = [nh] * 3
...
class FlowSequential(nn.Sequential):
    """ Sequential wrapper for normalizing flow layers. """
...
class CouplingNSF(nn.Module):
    ...
    """
    def __init__(self, dim, dimc=0, K=5, B=3, hidden_dim=8, base_network=FCN, device=0):
        super().__init__()
        self.dim = dim
        self.dimc = dimc # conditionals dim
        self.K = K # number of knots
        self.B = B # spline support
        self.conditioner = base_network(dim // 2 + self.dimc, (3 * K - 1) * (self.dim - (self.dim // 2)), hidden_dim)
        self.device = device
        self.to(self.device)

    def _get_spline_params(self, x1, c=None):
        x = x1 if c == None else torch.cat((x1, c), -1) # concat inputs
        # calls f(x_1:d), arrange spline params by input dim
        out = self.conditioner(x).view(-1, self.dim - (self.dim // 2), 3 * self.K - 1)
...
class NeuralSplineFlow(nn.Module):
    ...
    def sample(self, n, c=None):
        with torch.no_grad():
            ...
            x, _ = self.forward(z, c)
            return x
    
    def log_prob(self, x, c=None):
        """
        Compute the log likelihood of the input. The likelihood of z can be
